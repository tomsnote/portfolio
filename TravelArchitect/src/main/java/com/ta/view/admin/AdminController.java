...
public class AdminController {
	...
	public String flightManage(Model model,
			@RequestParam(value = "change", required = false, defaultValue = "flightList") String change,
		...
		if (change.equals("seatList")) {
			int listCnt = adminService.getSeatListCnt();
			pagination.pageInfo(page, range, listCnt);
			model.addAttribute("flightSeatList", adminService.getFlightSeatListAdmin(pagination));
		} else {
			int listCnt = adminService.getFlightListCnt();
			pagination.pageInfo(page, range, listCnt);
			model.addAttribute("flightList", adminService.getFlightListAdmin(pagination));
		}
	...
	public String moveInsertSeat(Model model, FlightVO vo,
			@RequestParam(value = "seatCheck", required = false) boolean seatCheck,
			@RequestParam(required = false, defaultValue = "1") int page,
			@RequestParam(required = false, defaultValue = "1") int range) {
	...
	public String insertSeat(FlightVO vo, RedirectAttributes redirectAttritributes) {
		List<FlightVO> seatList = adminService.getFlightSeatCheck();
	...
	public String q_and_a(Model model, @RequestParam(required = false, defaultValue = "1") int page,
			@RequestParam(required = false, defaultValue = "1") int range) {
